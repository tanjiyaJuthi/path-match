...
export default function CompanyProfile({ recruiter, recruiterCompany }) {
    ...
    const handleLogoUpload = async (e) => {
        ...
        try {
            ...
            if (!IMGBB_API_KEY) {
                throw new Error("Missing IMGBB API key (NEXT_PUBLIC_IMAGE_UPLOAD_API)");
            }
            ...
            if (!contentType?.includes("application/json")) {
                ...
                throw new Error(`Non-JSON response from imgbb: ${text}`);
            }
                        ...
                        <div className="flex flex-col gap-1 w-full">
                            ...
                            <div className="flex items-center gap-4 mt-1">
                                <label className="w-14 h-14 border border-dashed border-zinc-700 hover:border-zinc-500 bg-zinc-900/40 rounded-xl flex flex-col items-center justify-center cursor-pointer transition-colors group relative overflow-hidden">
                                    ...
                                    />
                                    {logoUrl ? (
                                        <Image width={100} height={100} src={logoUrl} alt="Logo Preview" className="w-full h-full object-cover" />
                                    ) : (
